...
def straightahead_reward_fn(
    ...
    straight_steering_threshold = STEERING_THRESHOLD * 0.5

    near_direction_diff = get_direction_diff(immediate_direction, lookahead_direction)
    far_direction_diff = get_direction_diff(immediate_direction, lookwayahead_direction)
    if (
        near_direction_diff <= straight_steering_threshold
        and far_direction_diff <= straight_steering_threshold
    ):
        straight_reward = speed / MAX_SPEED
        straight_reward += straight_steering_threshold / (
            straight_steering_threshold + steering
        )
    elif far_direction_diff <= straight_steering_threshold:
        straight_reward = speed / MAX_SPEED
...
def edge_reward_fn(
    ...
    if not is_left:
        if turn_direction == "right" and direction_diff >= STEERING_THRESHOLD * 3:
            edge_reward += direction_diff / (direction_diff + STEERING_THRESHOLD * 3)
            edge_reward += border_reward

            if not all_wheels_on_track:
                edge_reward += 0.4

        elif turn_direction == "left" and direction_diff > STEERING_THRESHOLD * 3:
            edge_reward = -direction_diff / (direction_diff + STEERING_THRESHOLD * 3)
        else:
            edge_reward += border_reward
            if not all_wheels_on_track:
