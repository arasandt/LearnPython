...
class Reward:
    ...
    def calc_direction_reward(self):
        ...
        self.upcoming_angle = max(self.upcoming_angle - self.direction_diff, 0)
        ...
        self.direction_reward = self.direction_reward / math.pow(
            self.upcoming_angle / 180, 2
        )

    def calc_speed_reward(self):
        speed_reward = self.speed / MAX_SPEED
        # speed_reward = 1.01 - (MAX_SPEED - self.speed) / 3

        # fast_speed_reward = math.pow(self.direction_reward_main, 2) * speed_reward
        slow_speed_reward = math.pow(self.upcoming_angle / 180, 2) * speed_reward

        # self.speed_reward = fast_speed_reward + slow_speed_reward
        self.speed_reward = slow_speed_reward
