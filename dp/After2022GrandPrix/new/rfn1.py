...
class Reward:
    ...
    def calc_direction_reward(self):
        ...
        self.direction_reward_base = Reward.DIRECTION_LIMIT / (
            Reward.DIRECTION_LIMIT + self.correct_direction_angle_diff
        )
        self.direction_reward = self.direction_reward_base * self.speed / MAX_SPEED

    def calc_steering_reward(self):
        self.steering_reward = MAX_STEERING / (MAX_STEERING + self.steering)
        self.steering_reward = self.steering_reward * self.speed / MAX_SPEED
    ...
    def calc_lane_reward(self):
        ...
        if self.uturn:
            if (self.turn_direction_uturn == "left" and self.is_left) or (
                ...
                self.direction_reward = self.direction_reward_base
            else:
                self.off_track = True
