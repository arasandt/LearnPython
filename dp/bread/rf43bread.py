...
DIRECTION_LOOKAHEAD = 4  # always + 2
...
class Reward:
    ...
    def __init__(
        ...
        (
            self.internal_behind_angle,
            self.internal_behind_turn_direction,
        ) = self.get_angle_and_turn(
            self.track_data["p" + str(DIRECTION_LOOKAHEAD)]["coordinates"],
            self.track_data["p" + str(int(DIRECTION_LOOKAHEAD // 2))]["coordinates"],
            self.track_data[0]["coordinates"],
        ...
        self.revised_direction_ahead = self.track_data[0]["angles"][
            DIRECTION_LOOKAHEAD * 2
        ]
    ...
    def calc_direction_reward(self):
        ...
        correct_direction_angle_ahead_diff = get_direction_diff(
            self.heading, self.revised_direction_ahead
        )

        acceptable_angle = get_direction_diff(
            self.revised_direction, self.revised_direction_ahead
        )
        current_position_angle = (
            correct_direction_angle_diff + correct_direction_angle_ahead_diff
        )
        error_angle = current_position_angle - acceptable_angle

        new_correct_direction_angle_diff = 0
        if (turn_direction == "right" and self.steering_angle < 0) or (
            turn_direction == "left" and self.steering_angle > 0
        ):
            new_correct_direction_angle_diff = abs(
                correct_direction_angle_diff - self.steering
            )
            new_error_angle = new_correct_direction_angle_diff - acceptable_angle

        if error_angle <= 0 or new_error_angle <= 0:
            self.direction_error = 1
        else:
            self.direction_error = 1 - error_angle / 90

        self.direction_error = max(0, self.direction_error)

        self.direction_reward = math.pow(self.direction_error, 1)
    ...
    def calc_speed_reward(self):
        ...
        # direction_weight = (self.upcoming_angle / 180 + self.current_angle / 180) / 2
        # self.direction_reward /= math.pow(direction_weight, 2)
