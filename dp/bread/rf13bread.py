...
class Reward:
    ...
    def __init__(
        ...
        if self.upcoming_angle <= CORRECT_LANE_THRESHOLD_DEGREE:
            new_direction = int(actual_direction * 1.5)
            self.revised_direction = self.track_data[0]["angles"][new_direction]
    ...
    def calc_speed_reward(self):
        self.speed_reward = (
            math.pow(abs(self.direction_reward), 2) * self.speed / MAX_SPEED
        )
        ...
        if (
            self.current_angle <= CORRECT_LANE_THRESHOLD_DEGREE
            or self.upcoming_angle <= CORRECT_LANE_THRESHOLD_DEGREE
        ):
            self.speed_reward = 1
            self.direction_reward = self.direction_reward * self.direction_reward
