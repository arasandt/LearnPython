...
def edge_reward_fn(
    ...
    if (steering_angle <= (-20 + STEERING_THRESHOLD * 1.5) and not is_left) or (
        ...
        edge_reward = 0.14
        if (all_wheels_on_track and is_left) or (
            not all_wheels_on_track and not is_left
        ):
            edge_reward += 0.04
        distance_from_border = 0.5 * track_width - distance_from_center
        if 0.05 < distance_from_border <= 0.25:
            edge_reward += 0.04
...
def reward_function(params):
    ...
    ######## Check #2 - OUTLIERS CHECK ########
        ...
        if turn_direction == "left" and direction_diff >= steering_angle >= -0.1:
            pass
        elif turn_direction == "right" and -direction_diff <= steering_angle <= 0.1:
            pass
        else:
            return reward
