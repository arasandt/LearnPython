...
class Reward:
    ...
    def calc_direction_reward(self):
        ...
        if self.turn_direction == "right" and self.steering_angle > direction_limit:
            self.steering_error = abs(self.steering_angle - direction_limit)
        ...
        #     self.steering_error += self.steering - self.direction_diff
        ...
        if self.steering_error >= Reward.DIRECTION_LIMIT:
            self.direction_reward_steering = 0.01
        else:
            self.direction_reward_steering = (
                1 - self.steering_error / Reward.DIRECTION_LIMIT
            )

        self.direction_reward_final = (
            self.direction_reward_main * self.direction_reward_main
        ) * 0.75 + (
            self.direction_reward_steering * self.direction_reward_steering
        ) * 0.25
    ...
    def calc_speed_reward(self):
        final_angle = min(
            self.upcoming_angle
            - self.correct_direction_angle_diff * 0.75
            - self.steering_error * 0.25,
            0,
        )
        self.speed_reward = math.pow(final_angle / 180, 2) * self.speed / MAX_SPEED
