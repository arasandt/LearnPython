...
DIRECTION_LOOKAHEAD = 10  # always + 2
...
class Track:
    ...
    def get_track_data(self):
        ...
        upcoming_waypoints = [
            ...
            for point in range(LOOKAHEAD_COVERAGE)
        ]
...
class Reward:
    ...
    def __init__(
        ...
        (
            self.half_current_angle,
            self.half_current_angle_turn_direction,
        ) = self.get_angle_and_turn(
            self.track_data["p" + str(int(DIRECTION_LOOKAHEAD // 2))]["coordinates"],
            self.track_data[0]["coordinates"],
            self.track_data[int(DIRECTION_LOOKAHEAD // 2)]["coordinates"],
        )
        (
            self.internal_angle,
    ...
    def calc_direction_reward(self):
        ...
        self.direction_error_one = 1 - self.correct_direction_angle_diff / 180
        self.direction_error_one = max(0, self.direction_error_one)
        self.direction_error_two = 1 - self.correct_direction_angle_diff / 135
        self.direction_error_two = max(0, self.direction_error_two)
        self.direction_error_three = 1 - self.correct_direction_angle_diff / 90
        self.direction_error_three = max(0, self.direction_error_three)
        self.direction_error_four = 1 - self.correct_direction_angle_diff / 45
        self.direction_error_four = max(0, self.direction_error_four)

        self.direction_reward = (
            self.direction_error_one
            + self.direction_error_two
            + self.direction_error_three
            + self.direction_error_four
        ) / 4
    ...
    def calc_speed_reward(self):
        ...
        if self.direction_diff <= 90:
            self.speed_reward = direction_weight * self.speed / MAX_SPEED
        else:
            self.speed_reward = (MAX_SPEED / self.speed) / direction_weight

        self.direction_reward = self.direction_reward / direction_weight

        # acceptable_angle = 10

        # if self.correct_direction_angle_diff > acceptable_angle:
        #     self.speed_reward = self.speed_reward / (
        #         2
        #         + (self.correct_direction_angle_diff - acceptable_angle)
        #         / acceptable_angle
        #     )

        #     if (
        #         self.half_current_angle_turn_direction == "right"
        #         and self.steering_angle > acceptable_angle
        #     ) or (
        #         self.half_current_angle_turn_direction == "left"
        #         and self.steering_angle < -acceptable_angle
        #     ):
        #         self.speed_reward = self.speed_reward / (
        #             2
        #             + (
        #                 self.correct_direction_angle_diff
        #                 - acceptable_angle
        #                 + self.steering
        #             )
        #             / acceptable_angle
    ...
    def calc_lane_reward(self):
        ...
        if self.half_current_angle < CORRECT_LANE_THRESHOLD_DEGREE:
            if (self.half_current_angle_turn_direction == "left" and self.is_left) or (
                self.half_current_angle_turn_direction == "right" and not self.is_left
            ):
                self.lane_reward = 1
            else:
                self.lane_reward = 0
