...
# }
# Action space from file: {'speed': {'high': 2.0, 'low': 1.2}, 'steering_angle': {'high': 22.5, 'low': -15}}
...
class Reward:
    ...
    def __init__(
        ...
        if self.uturn_angle < UTURN_THRESHOLD_DEGREE:
            self.uturn = True
        if self.uturn_angle < LANE_THRESHOLD_DEGREE:
            self.lane = True
    ...
    def calc_direction_reward(self):
        ...
        self.track_direction_angle_diff = get_direction_diff(
        ...
        #     self.direction_limit * 2
        # ):
        #     steering_error = abs(self.steering_angle - self.direction_limit * 2)
    ...
    def calc_speed_reward(self):
        self.speed_reward = 0.01
        if int(self.correct_direction_angle_diff) <= Reward.DIRECTION_LIMIT // 3:
        ...
        # self.uturn_speed_reward = slow_speed / (slow_speed + speed_diff)

        # cut down the speed reward based on how much direction diff it is beyond threshold.
        # if self.track_direction_angle_diff > MAX_STEERING * 1.5:
        # self.speed_reward = self.speed_reward * (
        #     Reward.DIRECTION_LIMIT
        #     / (self.track_direction_angle_diff + Reward.DIRECTION_LIMIT)
        # )
        # speed_diff = abs(SLOW_SPEED_ANCHOR - self.speed)
        # self.speed_reward = SLOW_SPEED_ANCHOR / (speed_diff + SLOW_SPEED_ANCHOR)
    ...
    # def slow_down(self):
    #     self.speed_reward = self.uturn_speed_reward
