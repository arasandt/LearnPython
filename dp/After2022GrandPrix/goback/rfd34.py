...
class Track:
    ...
    def get_track_data(self):
        ...
        # Get the angle from waypoint 1 to lookbehind waypoint for uturn
        lookbehind_waypoint_uturn = self.get_prev_waypoint(
            self.closest_waypoints[1], UTURN_WINDOW - UTURN_WINDOW_LOOKAHEAD
        )
        lookbehind_angle_uturn = get_angle_between_coordinates(
            track_data[UTURN_WINDOW_LOOKAHEAD]["coordinates"],
            self.waypoints[lookbehind_waypoint_uturn],
        )
        track_data["p99"] = {
            "waypoint": lookbehind_waypoint_uturn,
            "coordinates": self.waypoints[lookbehind_waypoint_uturn],
            "angles": [lookbehind_angle_uturn],
        }
        # check whether on a u-turn between lookbehind and lookahead
        track_data["uturn"] = get_direction_diff(
            lookbehind_angle_uturn,
            track_data[UTURN_WINDOW_LOOKAHEAD]["angles"][UTURN_WINDOW],
        )
...
class Reward:
    ...
    def __init__(
        ...
        uturn_direction_angle_one = get_angle_between_coordinates(
            ...
            self.track_data[UTURN_WINDOW_LOOKAHEAD]["coordinates"],
        )
        uturn_direction_angle_two = get_angle_between_coordinates(
            self.track_data["p99"]["coordinates"],
            self.track_data[UTURN_WINDOW + UTURN_WINDOW_LOOKAHEAD]["coordinates"],
        )
