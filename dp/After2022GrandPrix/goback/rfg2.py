...
class Track:
    ...
    def get_track_data(self):
        ...
        # Get the angle from waypoint 1 to lookbehind waypoint
        lookbehind_waypoint = self.get_prev_waypoint(self.closest_waypoints[1], 2)
        lookbehind_angle = get_angle_between_coordinates(
            track_data[3]["coordinates"],
            self.waypoints[lookbehind_waypoint],
        )
        track_data["upcominguturn2"] = get_direction_diff(
            lookbehind_angle,
            track_data[3]["angles"][5],
        )
...
class Reward:
    ...
    def get_revised_direction(self, current_coordinate, lookahead=20, limit=30):
        for point in list(range(lookahead + 1))[::-1][:-5:]:
            revised_direction = Reward.get_revised_direction_discrete(
                current_coordinate, self.track_data, lookahead=point
            )

            if int(get_direction_diff(revised_direction, self.heading)) <= limit:
                return revised_direction
        return self.track_data[0]["angles"][5]
    ...
    def __init__(
        ...
        self.slow = False
        self.uturn_angle = int(self.track_data["upcominguturn2"])
    ...
    def calc_direction_reward(self):
        ...
        revised_direction = self.get_revised_direction(self.heading_coordinates["xy"])
