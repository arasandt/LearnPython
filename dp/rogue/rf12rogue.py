...
DIRECTION_LOOKAHEAD = 3  # always + 2
...
class Reward:
    ...
    def __init__(
        ...
        self.anchor_direction = get_angle_between_coordinates(
            self.track_data["p" + str(DIRECTION_LOOKAHEAD)]["coordinates"],
            self.track_data[DIRECTION_LOOKAHEAD]["coordinates"],
        )
        self.forward_direction = self.track_data[0]["angles"][DIRECTION_LOOKAHEAD]

        ##### DIRECTION ENDS #####

    def calc_direction_reward(self):
        self.direction_reward = 0
        self.direction_error = 0
        ...
        # Where am I?
        angle_anchor_and_heading = get_direction_diff(
            self.anchor_direction, actual_heading
        )
        angle_anchor_and_heading_turn_direction = self.get_turn_direction(
            self.anchor_direction, actual_heading
        )

        angle_forward_and_heading = get_direction_diff(
            self.forward_direction, actual_heading
        )
        angle_forward_and_heading_turn_direction = self.get_turn_direction(
            self.forward_direction, actual_heading
        )

        angle_anchor_and_forward = get_direction_diff(
            self.anchor_direction, self.forward_direction
        )
        angle_anchor_and_forward_turn_direction = self.get_turn_direction(
            self.anchor_direction, self.forward_direction
        )

        if angle_anchor_and_forward <= 2:
            ...
            if (
                ...
                if (
                    ...
                    self.direction_error = self.steering / MAX_STEERING
                    # self.direction_error = (
                    #     1 + math.pow(angle_anchor_and_heading, 2) / 60
                    # )
                    if self.direction_error:
                        self.direction_error = self.speed / self.direction_error
                    else:
                        self.direction_error = 0
            else:
                self.direction_error = 0
    ...
    def calc_speed_reward(self):
        if self.direction_error:
            self.speed_reward = self.speed / self.direction_error
        else:
            self.speed_reward = 0
