...
# }
# Action space from file: {'speed': {'high': 2.0, 'low': 1.5}, 'steering_angle': {'high': 20, 'low': -15}}

import math, time

MAX_STEERING = 30
MIN_SPEED, MAX_SPEED = 1.5, 4.0
...
class Track:
    ...
    def get_track_data(self):
        ...
        lookbehind_waypoint = self.get_prev_waypoint(self.closest_waypoints[1], 7)
        ...
        track_data["uturn"] = get_direction_diff(
            lookbehind_angle,
            track_data[0]["angles"][7],
        )

        # check whether there is a uturn upcoming between lookahead and lookwayahead
        track_data["upcominguturn7"] = get_direction_diff(
            track_data[0]["angles"][7],
            track_data[7]["angles"][7],
        )
...
class Reward:
    ...
    def get_revised_direction(self, current_coordinate, track_data, lookahead):
        angles = []
    ...
    def __init__(
        ...
        self.border_reward = self.distance_from_center / (
            self.distance_from_center + 0.5 * track_width
        )
        ...
        self.upcoming_uturn = False
        if int(self.track_data["upcominguturn7"]) < UTURN_THRESHOLD_DEGREE:
            self.upcoming_uturn = True

    def calc_direction_reward(self):

        self.direction_limit = 1
        self.wrong_steering_limit = 5
        self.direction_factor = 1
        steering_error = 0
        lookahead = 15  # don't change this. 20 is not working.

        # revised_direction = self.track_data[0]["angles"][5]

        # revised_direction = self.get_revised_direction(
        #     self.track_data[0]["coordinates"], self.track_data, lookahead
        # )

        # revised_direction = self.get_revised_direction(
        #     self.heading_coordinates["xy"], self.track_data, lookahead
        # )

        direction = self.track_data[5]["coordinates"]
        if self.upcoming_uturn:
            direction = self.track_data[1]["coordinates"]

        revised_direction = get_angle_between_coordinates(
            self.heading_coordinates["xy"], direction
        )
        ...
        # steering error just to train faster
        if self.correct_turn_direction == "right" and self.steering_angle > (
            self.wrong_steering_limit
        ):
            steering_error = abs(self.steering_angle - self.wrong_steering_limit)
        elif self.correct_turn_direction == "left" and self.steering_angle < (
            -self.wrong_steering_limit
        ):
            steering_error = abs(self.steering_angle + self.wrong_steering_limit)

        # direction limit gives some freedom for training to decide the angle
        if self.correct_direction_angle_diff <= self.direction_limit:
            self.direction_factor = 1.25
        ...
        self.direction_reward *= self.direction_factor
    ...
    def calc_speed_reward(self):
        ...
        self.speed_reward *= self.speed_reward
    ...
    def calc_lane_reward(self):
        ...
        if self.uturn:
            if (
                (self.correct_turn_direction == "left" and self.is_left)
                or (self.correct_turn_direction == "right" and not self.is_left)
                # and self.border_reward > 0.25
            ):
