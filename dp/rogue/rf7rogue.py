...
class Reward:
    ...
    def calc_direction_reward(self):
        ...
        for count in range(2, DIRECTION_LOOKAHEAD):
            turn_direction = self.get_turn_direction(
                self.track_data[0]["angles"][1], self.track_data[0]["angles"][count]
            )
            direction_diff = get_direction_diff(
                self.track_data[0]["angles"][1], self.track_data[0]["angles"][count]
            )

            if turn_direction == "right":
                direction_factor += direction_diff
            else:
                direction_factor -= direction_diff

        direction_factor = abs(direction_factor)

        self.direction_error = 1 - correct_direction_angle_diff / 90

        self.direction_reward = max(0, self.direction_error * direction_factor)
