...
def parse_failure_history(failed_execution_arn):
    ...
    try:
        ...
        response = client.get_execution_history(
            executionArn=failed_execution_arn, reverseOrder=True
        )
    ...
    if failed_events[0]["executionFailedEventDetails"]["error"] == "States.Runtime":
        failed_id = int(
            filter(
                str.isdigit,
                str(
                    failed_events[0]["executionFailedEventDetails"]["cause"].split()[13]
                ),
            )
        )
        failed_state = failed_events[-1 * failed_id]["stateEnteredEventDetails"]["name"]
        failed_input = failed_events[-1 * failed_id]["stateEnteredEventDetails"][
            "input"
        ]
...
def attach_go_to_state(failed_state_name, state_machine_arn):
    ...
    try:
        response = client.create_state_machine(
            name=new_name, definition=json.dumps(state_machine), roleArn=role_arn
        )
    except Exception as cause:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Execution Arn of the failed state machine."
    )
    ...
    print(
        "Execution had failed at state: {} with Input: {}".format(
            failed_sm_state, failed_sm_info
        )
    )
