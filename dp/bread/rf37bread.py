...
class Reward:
    ...
    def calc_speed_reward(self):
        direction_weight = self.current_angle / 180
        self.direction_reward = self.direction_reward / direction_weight

        direction_factor = 2 + ((180 - self.current_angle) * 2) / 180

        self.speed_reward = self.speed / math.pow(
            max(1, self.correct_direction_angle_diff), direction_factor
        )
