...
class Reward:
    ...
    def calc_speed_reward(self):
        self.speed_reward = 1.001 - (MAX_SPEED - self.speed) / 3
        self.speed_reward = math.pow(self.direction_reward_temp, 2) * self.speed_reward

    def calc_lane_reward(self):
        self.lane_reward = 1

        current_coordinate = self.track_data[0]["coordinates"]
        five_coordinate = self.track_data[5]["coordinates"]
        ten_coordinate = self.track_data[10]["coordinates"]
        ...
        if upcoming_angle < SLOW_THRESHOLD_DEGREE:
            self.speed_reward = (MAX_SPEED - self.speed) / 3 + 0.001
            self.speed_reward = (
                math.pow(self.direction_reward_temp, 2) * self.speed_reward
            )
