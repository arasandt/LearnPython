...
def reward_function(params):
    ...
    ######## Check #2 - OUTLIERS CHECK ########
        ...
        reward = direction_reward_fn(immediate_direction, lookahead_direction, heading)

        ######## Check #3 - STEERING CHECK ########
        direction_diff = get_direction_diff(immediate_direction, lookahead_direction)
        # steering is within the direction difference angle, else no reward
        if steering <= direction_diff / 2:
            pass
        else:
            return 0
