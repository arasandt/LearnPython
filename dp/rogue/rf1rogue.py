...
DIRECTION_LOOKAHEAD = 5  # always + 2
...
class Reward:
    ...
    def calc_direction_reward(self):
        ...
        acceptable_angle = max(2, acceptable_angle)
        ...
        correct_direction_angle_diff = get_direction_diff(
            actual_heading, self.revised_direction
        )
        correct_direction_angle_ahead_diff = get_direction_diff(
            actual_heading, self.revised_direction_ahead
        )

        current_position_angle = (
            correct_direction_angle_diff + correct_direction_angle_ahead_diff
        )
        error_angle = max(0, current_position_angle - acceptable_angle)

        self.speed_divisor = 1 + error_angle / acceptable_angle
    ...
    def calc_speed_reward(self):
        self.speed_reward = (self.speed / MAX_SPEED) / self.speed_divisor
