...
class Reward:
    ...
    def __init__(
        ...
        # Get the angle and direction for upcoming waypoint from the heading
        self.upcoming_directions_from_heading()

        self.slow = False
        if self.track_data["uturn"] <= 110:
            self.slow = True

        self.upcoming_slow = False
        if self.track_data["upcominguturn"] <= 110:
    ...
    def direction_and_angle_between_two_lines(last_center, last, nex_center, nex):
        ...
        angle = get_direction_diff(first_line, second_line)
    ...
    def calc_direction_reward(self):
        ...
        if self.upcoming_slow:
            self.correct_turn_direction = self.upcoming_directions[0]["direction"]
            self.correct_direction_angle_diff = self.upcoming_directions[0][
                "direction_angle"
            ]
            self.lookahead_direction_diff = self.correct_direction_angle_diff
        else:
            self.correct_turn_direction = self.upcoming_directions[self.LOOKAHEAD][
                "direction"
            ]
            self.correct_direction_angle_diff = self.upcoming_directions[
                self.LOOKAHEAD
            ]["direction_angle"]
            self.lookahead_direction_diff = self.correct_direction_angle_diff
    ...
    def calc_lane_reward(self):
        ...
        # elif self.lookahead_direction_diff >= 75:
        #     self.slow_down()
