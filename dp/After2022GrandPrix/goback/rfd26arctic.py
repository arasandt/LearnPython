...
class Track:
    ...
    def get_track_data(self):
        ...
        lookbehind_waypoint = self.get_prev_waypoint(self.closest_waypoints[1], 15)
...
class Reward:
    ...
    def get_revised_direction_shortcut(self, limit=1):
        revised_direction = self.track_data[0]["angles"][15]
        save_direction_diff = get_direction_diff(
            revised_direction, self.track_data[0]["angles"][1]
        )

        for angle in self.track_data[0]["angles"][16:]:
    ...
    def __init__(
        ...
        uturn_direction_angle_two = get_angle_between_coordinates(
            self.track_data["p99"]["coordinates"], self.track_data[15]["coordinates"]
        )
