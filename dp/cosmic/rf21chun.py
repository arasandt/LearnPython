...
class Reward:
    ...
    def __init__(
        ...
        self.revised_direction = self.track_data[0]["angles"][6]
    ...
    def calc_direction_reward(self):
        ...
        if self.correct_direction_angle_diff >= Reward.DIRECTION_LIMIT:
            self.direction_reward_main = 0.001
        else:
            self.direction_reward_main = 1 - math.pow(
                self.correct_direction_angle_diff, 1
            ) / math.pow(Reward.DIRECTION_LIMIT, 1)

        if steering_error >= Reward.DIRECTION_LIMIT:
            self.direction_reward_steering = 0.001
        else:
            self.direction_reward_steering = 1 - math.pow(steering_error, 1) / math.pow(
                Reward.DIRECTION_LIMIT, 1
            )

        self.direction_reward = (
            self.direction_reward_main + self.direction_reward_steering
        )

    def calc_speed_reward(self):
        speed_reward = 1.001 - (MAX_SPEED - self.speed) / 3

        fast_speed_reward = math.pow(self.direction_reward_main, 2) * speed_reward

        current_coordinate = self.track_data[0]["coordinates"]
        five_coordinate = self.track_data[6]["coordinates"]
        ten_coordinate = self.track_data[12]["coordinates"]

        upcoming_angle = get_upcoming_angle(
            current_coordinate, five_coordinate, ten_coordinate
        )
        upcoming_angle = max(upcoming_angle - self.direction_diff, 0)

        slow_speed_reward = math.pow(upcoming_angle / 180, 2) * speed_reward

        self.speed_reward = fast_speed_reward + slow_speed_reward
    ...
    def calc_progress_reward(self, steps_threshold):
        self.progress_reward = 0.001
        if self.progress == 100:
            if self.steps < steps_threshold:
                self.progress_reward = (
                    2
                    * episode_reward.points
                    / self.steps
                    * (steps_threshold - self.steps)
                )
            episode_reward.points = 0
    ...
    def get_all_rewards(self, display=False):
        ...
        if self.steps == 1:
            ...
            episode_reward.points += total_rewards
...
def reward_function(params):
    ...
    if params["is_offtrack"] or params["is_reversed"]:
        ...
        reward.calc_progress_reward(400)
