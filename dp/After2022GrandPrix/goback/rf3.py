...
# }
# Action space from file: {'speed': {'high': 2.0, 'low': 1.0}, 'steering_angle': {'high': 25, 'low': -20}}

import math, time

MAX_STEERING = 30
MIN_SPEED, MAX_SPEED = 1.0, 2.0
SLOW_SPEED_ANCHOR = 1.5
...
class Reward:
    ...
    def get_revised_direction(self, current_coordinate, track_data, lookahead=15):
        angles = []
    ...
    def __init__(
        ...
        midline_reward = (0.5 * track_width) / (
            distance_from_center + 0.5 * track_width
        )
        self.border_reward = 1 - midline_reward
    ...
    def calc_direction_reward(self):
        ...
        if not self.upcoming_uturn:
            if (
                self.correct_turn_direction == "right"
                and self.steering_angle > direction_limit * 2
            ):
                steering_error = abs(self.steering_angle - direction_limit * 2)
            elif (
        ...
        errors = self.correct_direction_angle_diff + steering_error
        self.direction_reward = Reward.DIRECTION_LIMIT / (
            Reward.DIRECTION_LIMIT + math.pow(errors, 2)
        )
    ...
    def slow_down(self):
        ...
        self.calc_steering_reward(0.25)
