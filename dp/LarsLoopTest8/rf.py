...
def edge_reward_fn(
    ...
    wheels_on_track_factor = 1.25
    border_factor = 0.25

    if direction_diff <= STEERING_THRESHOLD:
        if is_left:  # speed on right lane
            edge_reward *= incorrect_lane_factor
        if all_wheels_on_track:  # change to distance from border
            edge_reward *= wheels_on_track_factor
    # right turn
    elif steering_angle <= (-20 + STEERING_THRESHOLD * 1.5):
        if is_left:  # right turn on right lane
            edge_reward *= incorrect_lane_factor
    # left turn
    elif steering_angle >= MAX_STEERING - (STEERING_THRESHOLD * 1.5):
        if not is_left:  # left turn on left lane
            edge_reward *= incorrect_lane_factor
        else:
        ...
        if all_wheels_on_track:  # because of U turn
            edge_reward *= wheels_on_track_factor  # change to distance from border
