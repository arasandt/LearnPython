...
class Track:
    ...
    def get_track_data(self):
        ...
        lookbehind_waypoint_direction = self.get_prev_waypoint(
            self.closest_waypoints[1], 5 * 2
        )
        lookbehind_angle_direction = get_angle_between_coordinates(
            track_data[0]["coordinates"],
            self.waypoints[lookbehind_waypoint_direction],
        )
        track_data["p98"] = {
            "waypoint": lookbehind_waypoint_direction,
            "coordinates": self.waypoints[lookbehind_waypoint_direction],
            "angles": [lookbehind_angle_direction],
        }
...
class Reward:
    ...
    def __init__(
        ...
        uturn_direction_angle_two_direction = get_angle_between_coordinates(
            self.track_data["p98"]["coordinates"], self.track_data[5 * 2]["coordinates"]
        )

        self.revised_direction = self.track_data[0]["angles"][5 * 2]
        # self.revised_direction = uturn_direction_angle_two_direction
    ...
    def calc_direction_reward(self):
        ...
        direction_reward_temp_1 = Reward.DIRECTION_LIMIT / (
            Reward.DIRECTION_LIMIT + self.direction_diff
        )
        direction_reward_temp_2 = Reward.DIRECTION_LIMIT / (
            Reward.DIRECTION_LIMIT + self.correct_direction_angle_diff
        )

        self.direction_reward_temp = (
            direction_reward_temp_1 + direction_reward_temp_2
        ) / 2
    ...
    def calc_lane_reward(self):
        ...
        if self.uturn:
            if (self.turn_direction_uturn == "left" and self.is_left) or (
                ...
                # self.speed_reward = self.uturn_angle
                # speed_reward = (MAX_SPEED - self.speed) / 3
                # self.speed_reward = (self.speed_reward * 0.5) + speed_reward * 0.5
            else:
                self.off_track = True
