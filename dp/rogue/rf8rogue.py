...
class Reward:
    ...
    def calc_direction_reward(self):
        ...
        # Where am I?
        angle_current_and_heading = get_direction_diff(
            self.track_data[0]["angles"][1], self.heading
        )
        angle_current_and_heading_turn_direction = self.get_turn_direction(
            self.track_data[0]["angles"][1], self.heading
        )

        angle_revised_and_heading = get_direction_diff(
            self.revised_direction, self.heading
        )
        # angle_revised_and_heading_turn_direction = self.get_turn_direction(
        #     self.revised_direction, self.heading
        # )

        angle_current_and_revised = get_direction_diff(
            self.track_data[0]["angles"][1], self.revised_direction
        )
        angle_current_and_revised_turn_direction = self.get_turn_direction(
            self.track_data[0]["angles"][1], self.revised_direction
        )

        # is correct direction?
        if (
            angle_current_and_heading_turn_direction
            == angle_current_and_revised_turn_direction
        ):
            if angle_current_and_heading > angle_current_and_revised:
                self.direction_error = 1

        if self.direction_error == 0:
            self.direction_error = 1 + angle_revised_and_heading

        if (
            angle_current_and_revised_turn_direction == "right"
            and self.steering_angle <= 0
        ) or (
            angle_current_and_revised_turn_direction == "left"
            and self.steering_angle >= 0
        ):
            pass
        else:
            self.direction_error *= 2
